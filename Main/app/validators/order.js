...
export const orderValidator = vine.compile(vine.object({
    userId: vine
        .number()
        .exists(async (db, value) => {
        return (db.from('users').where('id', value).andWhere('role_id', Roles.CUSTOMER).first() || false);
    })
        .optional()
    ...
    orderItems: vine.array(vine.object({
...
export const orderCalculationValidator = vine.compile(vine.object({
    type: vine.enum(['dine_in', 'delivery', 'pickup']),
    manualDiscount: vine.number().optional(),
    orderItems: vine.array(vine.object({
        menuItemId: vine.number().exists(async (db, value) => {
...
export const orderUpdateValidator = vine.compile(vine.object({
    userId: vine
        .number()
        .exists(async (db, value) => {
        return (db.from('users').where('id', value).andWhere('role_id', Roles.CUSTOMER).first() || false);
...
export const customUpdateValidator = vine.compile(vine.object({
    status: vine
        .enum((field) => {
        return getStatus(field.parent.type);
    })
...
export const bulkCustomUpdateValidator = vine.compile(vine.object({
    ids: vine
        .array(vine.number())
        .distinct()
        .compact()
...
function getStatus(type) {
    if (['dine_in', 'pickup'].includes(type)) {
        return ['pending', 'processing', 'ready', 'completed', 'canceled', 'failed'];
    }
    return [
        'pending',
        'processing',

...
import { bulkCustomUpdateValidator, bulkDeleteValidator, customUpdateValidator, reservationValidator, } from '#validators/reservation';
...
export default class ReservationsController {
    async index({ logger, request, response }) {
        ...
        try {
            const data = await Reservation.filter(input)
                ...
                if (listType === 'history')
                    return query.whereIn('status', ['cancelled', 'completed']);
            })
                .preload('user')
    ...
    async exportReservations({ logger, request, response }) {
        const { listType, page, limit, ...input } = request.qs();
        try {
...
//# sourceMappingURL=reservations_controller.js.map
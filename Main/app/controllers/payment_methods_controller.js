...
export default class PaymentMethodsController {
    async adminView({ logger, response, inertia }) {
        try {
    ...
    async index({ logger, request, response }) {
        ...
        try {
            ...
            const data = await PaymentMethod.filter(input)
                .where('status', true)
                .if(country, (query) => query.whereRaw('JSON_CONTAINS(countries, ?)', [JSON.stringify(country?.toUpperCase())]))
                .if(currencyCode, (query) => query.whereRaw('JSON_CONTAINS(currencies, ?)', [
                JSON.stringify(currencyCode.toUpperCase()),
            ]))
                .orderBy('createdAt', 'desc');
            const content = data.map((ele) => ele.serialize({
                fields: {
    ...
    async adminIndex({ logger, request, response }) {
        const { page, limit, ...input } = request.qs();
        try {
    ...
    async getDetails({ logger, request, response }) {
        const { id } = request.params();
        try {

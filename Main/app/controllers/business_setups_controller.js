...
import { businessInfoValidator, platformSetupValidator, siteSettingValidator, } from '#validators/business_setup';
import { attachmentManager } from '@jrmc/adonis-attachment';
export default class BusinessSetupsController {
    async getDetail({ logger, response }) {
        try {
    ...
    async updateBusinessInfo({ logger, request, response }) {
        try {
    ...
    async updatePlatformSetup({ logger, request, response }) {
        try {
    ...
    async updateSiteSetting({ logger, request, response }) {
        try {
            const payload = await request.validateUsing(siteSettingValidator);
            const { logo, minimizedLogo, favicon, aboutUsImage, contactUsImage, ...restPayload } = payload;
...
//# sourceMappingURL=business_setups_controller.js.map
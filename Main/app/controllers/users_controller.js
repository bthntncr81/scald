...
import { bulkCustomValidator, bulkDeleteValidator, customUpdateValidator, userProfileUpdateValidator, userUpdateValidator, userValidator, } from '#validators/user';
...
export default class UsersController {
    ...
    async exportUsers({ logger, request, response }) {
        ...
        try {
            ...
            response.header('Content-Disposition', `attachment; filename=${type === 'customer' ? 'customers.csv' : 'delivery.csv'}`);
    ...
    async getById({ logger, request, response }) {
        try {
    ...
    async store({ logger, request, response }) {
        try {
            ...
            if ([Roles.DELIVERY, Roles.CUSTOMER].includes(payload.roleId)) {
                ...
                await mail.sendLater(new UserCreateNotification(user, password, {
    ...
    async updateByUser({ logger, request, response, auth }) {
        try {
    ...
    async customUpdate({ logger, request, response, params, auth }) {
        try {
    ...
    async bulkDelete({ logger, request, response }) {
        try {

...
export default class SettingsController {
    async getPaymentOptions({ logger, response }) {
        try {
    ...
    async updatePaymentOptions({ logger, request, response }) {
        try {
            const payload = await request.validateUsing(updatePaymentOptionsValidator);
            const data = await Setting.updateOrCreate({ key: payload.name }, {
    ...
    async updateTheme({ logger, request, response }) {
        try {
            const payload = await request.validateUsing(updateThemeValidator);
            const data = await Setting.updateOrCreate({ key: payload.key }, {

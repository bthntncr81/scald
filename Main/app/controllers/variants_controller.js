...
import { bulkCustomUpdateValidator, bulkDeleteValidator, customUpdateValidator, variantValidator, } from '#validators/variant';
...
export default class VariantsController {
    ...
    async exportVariants({ logger, request, response }) {
        const { page, limit, ...input } = request.qs();
        try {
        ...
        try {
            ...
            if (payload.variantOptions) {
                ...
                for (const optionPayload of payload.variantOptions) {
                    if (optionPayload.id) {
                        const existingOption = variant.variantOptions.find((option) => option.id === optionPayload.id);
                        if (existingOption) {
    ...
    async delete({ logger, params, response }) {
        const { id } = params;
        try {
...
//# sourceMappingURL=variants_controller.js.map
...
export default class AuthController {
    async loginView({ inertia }) {
        return inertia.render('Auth/Auth');
    }
    async signupView({ inertia }) {
        return inertia.render('Auth/Auth');
    }
    async forgotPasswordView({ inertia }) {
        return inertia.render('Auth/ForgotPassword');
    }
    async newPasswordView({ inertia }) {
        return inertia.render('Auth/NewPassword');
    }
    async register({ logger, request, response }) {
        try {
            ...
            return response.json({
                success: true,
                message: 'Registration was successful. We sent a verification link to your email. Please check your email to verify your account.',
                user,
            });
        }
        catch (error) {
    ...
    async verifyEmail({ request, inertia }) {
        try {
    ...
    async login({ logger, auth, request, response }) {
        ...
        try {
            ...
            if (branding.business.loginOnlyVerifiedEmail && !user.isEmailVerified) {
                ...
                await mail.sendLater(new VerifyEmailNotification(user, newToken, {
                    business: branding.business,
                    siteUrl: branding.siteUrl || '',
                }));
                return response.json({
    ...
    async forgotPassword({ logger, request, response }) {
        try {
    ...
    async resetPassword({ logger, request, response }) {
        try {

...
export default class PromotionsController {
    async getDetail({ logger, request, response }) {
        try {
            ...
            return response.json({
                content: data.map((ele) => ele.serialize({ fields: { pick: ['id', 'type', 'sliderImage'] } })),
            });
        }
        catch (error) {
    ...
    async updatePromotion({ logger, request, response }) {
        try {
    ...
    async updateSlider({ logger, request, response }) {
        try {
            const sliderPayload = [];
            ...
            const content = await Promise.all(payload.slider.map(async (ele) => {

...
export default class PaymentsController {
    ...
    async capturePayPalOrder({ request, response, auth }) {
    ...
    async cancelPaypalOrder({ request, response }) {
        const { token } = request.qs();
        try {
            const order = await Order.query()
                .whereRaw("JSON_EXTRACT(payment_info, '$.id') = ?", [token])
    ...
    async retrieveStripeSession({ request, response, auth }) {
    ...
    async cancelStripeSession({ request, response }) {
        const { session_id: sessionId } = request.qs();
        try {
            const order = await Order.query()
                .whereRaw("JSON_EXTRACT(payment_info, '$.id') = ?", [sessionId])

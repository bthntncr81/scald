...
import { bulkCustomUpdateValidator, customUpdateValidator, orderCalculationValidator, orderUpdateValidator, orderValidator, } from '#validators/order';
...
export default class OrdersController {
    ...
    async adminIndex({ logger, request, response, auth }) {
    ...
    async exportOrders({ logger, request, response }) {
        const { page, limit, ...input } = request.qs();
        try {
    ...
    async orderStatusCount({ logger, request, response }) {
    ...
    async getById({ logger, response, params }) {
        try {
    ...
    async generateInvoice({ logger, response, params, view, auth }) {
        try {
            ...
            const orderItems = order.orderItems.map((ele) => {
                const data = ele.serialize({ fields: { emit: ['variants', 'addons', 'charges'] } });
                let parsedCharges = JSON.parse(ele.charges) || [];
                const aggregatedCharges = parsedCharges.reduce((acc, charge) => {
            ...
            const formattedDate = DateTime.fromISO(order.createdAt?.toString() || '').toFormat('yyyy-MM-dd');
    ...
    async calculate({ logger, request, response }) {
        try {
            ...
            const { total, totalQuantity, discount, totalTax, totalCharges, orderItemsData, chargesData, } = await this.processOrderItems(payload.orderItems);
            const grandTotal = total + totalTax + totalCharges + deliveryCharge - discount - (payload.manualDiscount || 0);
            if (grandTotal < 0) {
                return response.badRequest({
                    success: false,
    ...
    async store({ logger, request, response, auth }) {
        try {
            ...
            const deliveryCharge = payload.type === 'delivery' ? businessSetup.deliveryCharge : 0;
            const { total, totalQuantity, discount, totalTax, totalCharges, orderItemsData, chargesData, } = await this.processOrderItems(orderItems);
            let grandTotal = total + totalTax + totalCharges + deliveryCharge - discount - (payload.manualDiscount || 0);
            if (grandTotal < 0) {
                return response.badRequest({
                    success: false,
                    message: "Grand total can't be less than zero",
            ...
            if (auth.user.roleId === Roles.CUSTOMER && !['cash', 'card'].includes(payload.paymentType)) {
                ...
                if (payload.paymentType === 'paypal') {
                    ...
                    await data
                        ...
                        .save();
                    const redirectUrl = paypalOrderData.links.find((link) => link.rel === 'approve').href;
    ...
    async update({ logger, request, response, params }) {
        try {
            const order = await this.fetchOrderWithRelations(params.id);
    ...
    async customUpdate({ logger, request, response, params }) {
        try {
    ...
    async bulkCustomUpdate({ logger, request, response }) {
        try {
    ...
    async delete({ logger, request, response }) {
    ...
    async processSingleOrderItem(item) {
        ...
        const { itemTax, itemCharge, itemCharges } = this.calculateItemCharges(menuItem, basePrice + variantPrice + addonPrice - itemDiscount, item.quantity);
    ...
    async calculateVariants(variants, menuItem) {
        return Promise.all(variants.map(async (ele) => {
    ...
    async calculateAddons(addons, menuItem) {
        return Promise.all(addons.map(async (ele) => {
    ...
    calculateItemCharges(menuItem, basePrice, quantity) {
        let itemTax = 0;
        let itemCharge = 0;
        const itemCharges = [];
        menuItem.charges.forEach((charge) => {
            const amount = charge.amountType === 'percentage' ? basePrice * (charge.amount / 100) : charge.amount;
            if (charge.type === 'tax') {
                itemTax += amount;
    ...
    calculateDiscount(menuItem, basePrice, variantPrice) {
    ...
    calculateVariantPriceFromItems(items) {
        return items.reduce((sum, item) => sum + (item?.price || 0), 0);
    }
    calculateAddonPriceFromItems(items) {
        return items.reduce((sum, item) => sum + (item?.grandPrice || 0), 0);
    }
    async fetchOrderWithRelations(orderId) {
        return Order.query()
            .preload('user')
            .preload('orderItems', (query) => {
            query.preload('menuItem');

...
class NotificationService {
    ...
    async sendDeliveryManAssignedNotification(order) {
        try {
    ...
    async sendUserSuspentionNotification(user, AdminUser) {
        try {
            const admins = await User.query().whereIn('roleId', [Roles.ADMIN, Roles.MANAGER, Roles.POS]);
            admins.forEach(async (admin) => {
    ...
    async sendCreateReservationNotification(reservation, AdminUser) {
        try {
            const admins = await User.query().whereIn('roleId', [Roles.ADMIN, Roles.MANAGER, Roles.POS]);
            admins.forEach(async (admin) => {
                if (admin.id === AdminUser.id)
    ...
    async sendReservationStatusUpdateNotification(reservation, AdminUser) {
        try {
            const admins = await User.query().whereIn('roleId', [Roles.ADMIN, Roles.MANAGER, Roles.POS]);
            admins.forEach(async (admin) => {
                if (admin.id === AdminUser.id)
